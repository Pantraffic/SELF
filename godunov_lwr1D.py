...
def bound_cond_entry(k_prev, q_en, k_max, v_max, k_cr, q_max, fd):
    ...
    if q_en <= supply:
        ...
        # Handle entering at the exit boundary
        k = k_prev
    return k
...
def bound_cond_exit(k_prev, q_ex, k_max, v_max, k_cr, q_max, fd):
    ...
    if q_ex < demand:
        # Handle exiting at the entry boundary
        k = k_prev
    else:
...
run = np.random.uniform
rin = np.random.randint
train_cases = ['sic1', 'sic2', 'sic3', 'sic3']
...
for sc in train_cases:
    ...
    with open('train/train_lwrg20x1gs_{}.pkl'.format(sc), 'wb') as f:
        pkl.dump(data, f, protocol=pkl.HIGHEST_PROTOCOL)
    data = {'X':TestX, 'Y':TestY}
    with open('train/vald_lwrg20x1gs_{}.pkl'.format(sc), 'wb') as f:
        pkl.dump(data, f, protocol=pkl.HIGHEST_PROTOCOL)
